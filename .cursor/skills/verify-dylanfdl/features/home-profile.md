...
- **Confirm Muse Spark href.** Read the Muse Spark link `href` or click it and screenshot the destination. That is enough for a href claim. Do not film the navigation.
- **Confirm Muse Code href.** Same as above for the Muse Code link. On current main both Muse links share `https://research.meta.ai/blog/introducing-muse-code-and-muse-spark-1-2` until a version split lands.
...
- Muse Spark version copy is not a constant. Assert what the launched page shows; do not fail a home-profile proof because this file still mentions 1.2 after a later bump.
